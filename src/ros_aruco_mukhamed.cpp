...
//cap.set(CV_CAP_PROP_FRAME_HEIGHT,height);
    ...
    if(!cap.isOpened()) {
        ...
        while(ros::ok()){
            ...
            image = cv::imread("/home/issai/catkin_ws/src/ros_aruco/src/test.jpeg", CV_LOAD_IMAGE_COLOR); // get a new frame from camera
            // sharpen the frame
            ...
            if (!ids.empty()) {
                ...
                for(unsigned long i=0; i<ids.size(); ++i){
                    ...
                    std::cout << "w: " << q.w() << "; x: " << q.x() << std::endl;
                    std::cout << "y: " << q.y() << "; z: " << q.z() << std::endl;
                    ...
                    std::cout << "Rotation Matrix: " << Rm << std::endl;
                    ...
                    std::cout << "cmra matrx: " << cameraMatrix << std::endl;
                    std::cout << "dist coef: " << distCoeffs << std::endl;
                ...
                std::cout << "------------------" << std::endl;
                if((ps.point.y < y_max && ps.point.y > y_min) || (ps.point.y < 6.2 && ps.point.y > 5.7) || (ps.point.y < 10.7 && ps.point.y > 10.2)){
                    pose_pub.publish(ps);
...
ArUcoNode::ArUcoNode():n_h_("") {
    ...
    cameraMatrix = (cv::Mat_<float>(3,3) << 2968.248185769887, 0.000000, 2066.575474415749, 0.000000, 2969.689947522676, 1480.388096089323, 0.000000, 0.000000, 1.000000);
    distCoeffs = (cv::Mat_<float>(1,5) << -0.0048, 0.014, 0.0969, -0.3043, 0.2844 );
    
   // cameraMatrix = (cv::Mat_<float>(3,3) << 1412.7838859195074, 0.000000, 538.5406687372522, 0.000000, 1415.5341529909426, 967.9191919494591, 0.000000, 0.000000, 1.000000);
    std::cout << "cmra matrx: " << cameraMatrix << std::endl;
   // distCoeffs = (cv::Mat_<float>(1,5) << 0.22157127062124501, -0.5837844007081326, 0.00575747461456335, -0.0004400557512713847, 0.5473168889977913);
