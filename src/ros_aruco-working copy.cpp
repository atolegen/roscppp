...
#define SERVER_PORT htons(10006)
using namespace std;
...
int ArUcoNode::arucoDetect() {
	...
	cout<<"Hello"<<endl;
        ...
        while(ros::ok()){
	        ...
	        string token[100];
	        vector<int> ids(1);
	        vector<vector<cv::Point2f>> corners(1, vector<cv::Point2f>(4));
	    ...
	    if(token[0]!="00"){
	        //cout<<"Size of token"<<sizeof(token)/sizeof(token[0])<<endl;
	        //for(int i=0;i<sizeof(token)/sizeof(token[0]);i++){
        	corners[0][0] = cv::Point2f(stof(token[1]),stof(token[2]));
        	corners[0][1] = cv::Point2f(stof(token[3]),stof(token[4]));
		    corners[0][2] = cv::Point2f(stof(token[5]),stof(token[6]));
		    corners[0][3] = cv::Point2f(stof(token[7]),stof(token[8]));
		    ids[0]=stoi(token[0]);
    			
		    cout<<"ID:"<<ids[0]<<";Corners:";
		    for (int j = 0; j < 4; j++){ 
			    cout<< corners[0][j]<< ":"; 
		    } 
		    cout<< "\n"; 
		    //}
                ...
                cout << "------------------" << std::endl;
                //if(ps.point.y < y_max && ps.point.y > y_min){///////////IA KOMMENTIL. ETU NADO OTKALIBROVAT 
                    pose_pub.publish(ps);
...
ArUcoNode::ArUcoNode():n_h_("") {
    ...
    cameraMatrix = (cv::Mat_<float>(3,3) << 1.07722650e+03, 0.00000000e+00, 4.16768656e+02, 0.00000000e+00, 1.07778246e+03, 1.92345914e+02, 0.00000000e+00, 0.00000000e+00, 1.00000000e+00);//1.08440106e+03, 0.00000000e+00, 3.81902987e+02,0.00000000e+00, 1.08526972e+03, 1.89386463e+02,0.00000000e+00, 0.00000000e+00, 1.00000000e+00);
    //1378.438623, 0.000000, 924.854927, 0.000000, 1381.202474, 560.433664, 0.000000, 0.000000, 1.000000);

    distCoeffs = (cv::Mat_<float>(1,5) << -4.10713475e-02,  1.24356039e+00, -4.95145175e-03,  5.52341740e-03, -1.12065158e+01);
